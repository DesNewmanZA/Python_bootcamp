...
# Use the provided classes to remake the coffee machine
machine_on = True
menu = Menu()
...
while machine_on:
    ...
    if option == 'off':
        ...
        # Test if enough resources to make the coffee selected
        can_make = coffee_machine.is_resource_sufficient(drink)

        if can_make:
            # Take in money - assumes integer input
            solvent = money_machine.make_payment(drink.cost)
            
            # Only make if solvent
            if solvent:
