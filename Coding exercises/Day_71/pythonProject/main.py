...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        # If user id is not 1 then throw a 403 error
        if current_user.id != 1 or not current_user.is_authenticated:
            return abort(403)
        # Else continue with it as is
...
# Create database for blog posts and users
...
def register():
    ...
    if form.validate_on_submit():
        # Check if user already exists and if so alert to log in
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # If password is wrong
            ...
            return redirect(url_for('login'))
        # If registered and password exists
        else:
            login_user(user)
            return redirect(url_for('get_all_posts'))
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post, current_user=current_user,
                           form=comment_form, gravatar=gravatar_url(current_user.email))
